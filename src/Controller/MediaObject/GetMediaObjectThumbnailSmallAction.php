...
final class GetMediaObjectThumbnailSmallAction
{
    ...
    {
        if (is_null($mediaObject->getPathThumbnailSmall()) || $mediaObject->getPathThumbnailSmall() === '') {
            return;
        }

        try {
            $result = $this->awsS3Service->downloadFile($mediaObject->getPathThumbnailSmall());
        } catch (\Exception $e) {
